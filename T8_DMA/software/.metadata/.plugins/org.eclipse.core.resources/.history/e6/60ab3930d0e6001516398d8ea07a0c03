...
#define mycos(A) __builtin_custom_fnf(ALT_CI_COS_0_N,(A))
#define myadd(A,B) __builtin_custom_fnff(ALT_CI_DSD_FP_ALT_ADD_0_N,(A),(B))
#define mymult(A,B) __builtin_custom_fnff(ALT_CI_DSD_FP_ALT_MULT_0_N,(A),(B))
...
float sumVector_cos_custom(float x[], unsigned int M)
{
	...
	int i=0;
	for (i=0; i<M; i++){
		x_squared=mymult(x[i],x[i]);
		cos_out=mycos(x[i]);
		mult_out1=mymult(x[i],0.5);
