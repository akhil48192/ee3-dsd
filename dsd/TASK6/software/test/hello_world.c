...
#define MY_COS(A) __builtin_custom_ini(ALT_CI_COS_0_N,(A))
#define MY_FLOOR(A) __builtin_custom_ini(ALT_CI_COS_ROM_0_N,(A))
#define MY_ADD(A,B) __builtin_custom_fnff(ALT_CI_DSD_FP_ALT_ADD_0_N,(A),(B))
#define MY_MULT(A,B) __builtin_custom_fnff(ALT_CI_DSD_FP_ALT_MULT_0_N,(A),(B))
#define MY_MULT_i(A,B) __builtin_custom_inff(ALT_CI_DSD_FP_ALT_MULT_0_N,(A),(B))
...
float sumVector_cos_custom(float x[], unsigned int M)
{
	...
	float x_half=0.0;
	int i=0;
	for (i=0; i<M; i++){//x/2+xxcos(fx/4-32)
		x_half=MY_MULT(x[i],0.5);
		x_squared=MY_MULT(x[i],x[i]);
		floored = MY_FLOOR(MY_MULT_i(x[i], (const)0.25)-32);
		cos_out=cosf(floored);
		mult_out2=MY_MULT(x_squared,cos_out);
		result = MY_ADD(x_half, mult_out2);
	}
	return result;
}
