...
float sumVector_cos(float x[], unsigned int M)
{
	...
	int i=0;
	for (i=0; i<M; i++){
		result += (x[i]/2) + (x[i]*x[i]*cosf(f(x[i]/4)-32));
	}
	return result;
}
/*
float sumVector_cos(float x[], double xd[], unsigned int M)
{
	float result1=0, result2=0, result5=0;
	double result3=0, result4=0;
	int i=0;
	for (i=0; i<M; i++){
		//ff df fd dd ff
		result1 += (x[i] + x[i]*x[i]);
		result2 += (xd[i] + xd[i]*xd[i]);
		result3 += (x[i] + x[i]*x[i]);
		result4 += (xd[i] + xd[i]*xd[i]);
		result5 += (x[i] + x[i]*x[i]);
		if (i==M-1) printf("ind: %d, ff: %f, df: %f, fd: %f, dd: %f, naive: %f\n", i, result1, result2, result3, result4, result5);
	}
	return result1;
}
*/
